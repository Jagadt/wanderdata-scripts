...
p <- ggplot(df, aes(x=start.hour)) +
  ...
  labs(title="Sleeping start hour histogram",
...
p <- ggplot(df, aes(x=end.hour)) +
  ...
  theme(axis.title = element_text(size = 18), 
        plot.margin = unit(c(1.0, 1.0, 1.0, 0.5), 'cm'),
        axis.text.x = element_text(hjust = 1)) +
  labs(title="Sleeping end hour histogram",
       subtitle = sprintf("From %s until %s", first.date, last.date)) 
...
p <- ggplot(df, aes(x=minutesAsleep)) +
     ...
     theme(axis.title = element_text(size = 18), 
          plot.margin = unit(c(1.0,1.0,1.0,0.5), "cm"),
          axis.text.x = element_text(angle = 45, hjust = 1)) +
     labs(title="Minutes slept distribution",
         subtitle = sprintf("From %s until %s", first.date, last.date))
print(p)
```
```{r}
mode(trunc(df$minutesAsleep/60))
```
...
p <- ggplot(df, aes(x=minutesAwake)) +
     ...
     labs(title="Minutes awake histogram",
         subtitle = "Time elapsed before falling asleep")
print(p)
```
...
p <- ggplot() +
    ...
    theme(axis.title = element_text(size = 18), 
          plot.margin = unit(c(1.0,1.0,1.0,0.5), "cm"),
          axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(title="Starting sleeping time",
         subtitle = sprintf("From %s until %s", first.date, last.date)) +
    ylab("Hour") +
    xlab("Date")
print(p)
