...
p <- ggplot() +
          ...
          labs(title="Pressure altitude of visited places (in meters)",
               subtitle = sprintf("From %s until %s", first.date, last.date)) +
          xlab('Date') + ylab('Pressure altitude') +
          bbc_style() +
          theme(axis.title = element_text(size = 18), 
                    plot.margin = unit(c(1.5,1.5,1.5,1.5), "cm"),
                    axis.text.x = element_text(angle = 45, hjust = 1)) +
...
spotify.df <- spotify.df[spotify.df$dateTime >= args[1] & spotify.df$dateTime <= args[2],]
...
sleep.df <- sleep.df[sleep.df$dateTime >= args[1] & sleep.df$dateTime <= args[2],]
